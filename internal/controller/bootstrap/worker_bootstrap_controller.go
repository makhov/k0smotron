...
& $dest --version`, scope.Config.Spec.Version, scope.Config.Spec.Version, k0sPath, scope.Config.Spec.K0sInstallDir),
	...
	commands = append(commands, `C:\bootstrap\k0s_install.ps1`)
	...
	commands = append(commands, strings.Join(installCmdParts, " "))
	commands = append(commands, fmt.Sprintf(`& %s start`, k0sPath))
