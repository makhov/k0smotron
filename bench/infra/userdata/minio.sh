#!/bin/bash
# MinIO storage node: joins the mgmt k0s cluster as a tainted worker. The
# actual minio process runs as a pod scheduled here via nodeSelector
# db=minio + toleration for dedicated=minio:NoSchedule. The io2 data disk
# is mounted at /var/lib/minio-data and exposed to the pod via a Local PV.
#
...
echo "UUID=$DATA_UUID /var/lib/minio-data ext4 defaults,noatime 0 2" >> /etc/fstab
mount -a

# MinIO official image runs as uid 1000 (minio).
chown 1000:1000 /var/lib/minio-data
chmod 700 /var/lib/minio-data
...
k0s install worker \
  ...
  --kubelet-extra-args="--register-with-taints=dedicated=minio:NoSchedule --node-labels=db=minio"
...
k0s start
echo "MinIO storage node joined."
